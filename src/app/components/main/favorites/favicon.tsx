...
export default function RadioFavicon({radio, playingRadio, setPlayingRadio, isHover}:Arguments) {
  function showPlay(radio: RadioStation){
    if(isHover(radio)){
      if(playingRadio.stationuuid === "" || playingRadio.stationuuid !== radio.stationuuid){
        return true;
      }
    }
    return false;
  ...
  return (
    <div className={"col-xs-2 " + styles.radioFaviconDiv}>
      <img className={styles.listRadioFavicon + (hideFavicon(radio) ? " " + styles.transparentFavicon : "")} src={radio.favicon} alt={"favicon of " + radio.name} />
      <i onClick={() => setPlayingRadio(radio)} className={"bi bi-play-fill " + styles.radioPlayButton + (showPlay(radio) ? " visible" : " invisible")}></i>
      <i onClick={() => setPlayingRadio(new RadioStation())} className={"bi bi-stop-fill " + styles.radioStopButton + (showStop(radio) ? " visible" : " invisible")}></i>
    </div>
  );
}